...
class MembersController extends AsyncNotifier<List<Member>> {
  MembersController(this._tripRid);
  final String _tripRid;

  MemberRepository get _repo => ref.read(memberRepositoryProvider);

  @override
  Future<List<Member>> build() => _repo.list(_tripRid);

  Future<void> addGhost(String displayName, String role,
      {String? email}) async {
    await _repo.addGhost(_tripRid, displayName, role, email: email);
    ref.invalidateSelf();
    await future;
  }

  Future<void> remove(String memberRid) async {
    await _repo.remove(_tripRid, memberRid);
    ref.invalidateSelf();
    await future;
  }

  Future<void> changeRole(String memberRid, String role) async {
    await _repo.update(_tripRid, memberRid, role: role);
    ref.invalidateSelf();
    await future;
  }
  ...
  Future<void> editGhost(String memberRid,
    ...
    await _repo.update(_tripRid, memberRid,
  ...
  Future<void> merge(String sourceRid, String targetRid) async {
    await _repo.merge(_tripRid, sourceRid, targetRid);
    ref.invalidateSelf();
    await future;
  }
}
